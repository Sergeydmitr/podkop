...
### Безопасность
- Исправлена инъекция через невалидированные URL в wget/curl
- Добавлена валидация host/port при парсинге proxy-строк
- Добавлена валидация элементов перед добавлением в nft-наборы
- GitHub Actions закреплены по SHA коммитов (защита от supply chain атак)
- Docker-образы закреплены по digest (IPK)
- Добавлена проверка целостности скачиваемого конфига в install.sh

### Исправления ошибок
- Исправлена инвертированная логика проверки FakeIP в диагностике
- Исправлена переменная-призрак `$http_proxy_address` (должна быть `$service_proxy_address`)
- Исправлена ловушка subshell в проверке nft mark (переменные не передавались из подоболочки)
- Исправлена утечка памяти в MutationObserver (`onMount`) — добавлен таймаут 30с
- Добавлен try-catch для JSON.parse в WebSocket-обработчиках и парсинге outbound_json

### Улучшения
- Минимальная версия sing-box поднята до 1.13.2
- Корректный парсинг версий sing-box с суффиксами (например, `1.13.2-extended-1.6.2`)
- Добавлена политика автоперезапуска сервиса через procd (respawn)
- Добавлена валидация `badwan_reload_delay` на числовое значение
